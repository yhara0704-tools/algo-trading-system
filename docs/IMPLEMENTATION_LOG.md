...
## 2026-05-02 Day 8 — GW 残期間「あと一押し」3 連発 (健全性監査拡大 / 6752 強化 / 1m パイプ整備)

### D8a: 全戦略 60 日健全性監査 (Day 6 拡大)

`scripts/d8_health_check_all_strategies.py`:
universe の MacdRci 以外 (Breakout / BbShort / Pullback / EnhancedMacdRci) を
60 日 5m で再 backtest し、universe oos_daily との乖離を検出。

| 戦略 | n | real | oos_sum | ratio | 評価 |
|------|--:|-----:|--------:|------:|------|
| EnhancedMacdRci | 1 | +3,661 | +12,168 | 0.30 | OK (過大評価) |
| Breakout | 3 | -223 | +7,091 | -0.03 | **構造的負け** |
| Pullback | 4 | +1,222 | +4,214 | 0.29 | 部分的 |
| BbShort | 3 | +1,335 | +1,335 | **1.00** | **完璧** |

**衝撃発見**:
- **3103.T Breakout** (lot_mult=2.72!) は実測 -244 円/日 = 完全に逆!
- **9984.T EnhancedMacdRci** (lot_mult=3.0!) は実測 +3,661 (oos の 30%) = 大過大評価
- **BbShort 3 件** は oos と実測が完全一致 (1.00) = **最も信頼できる戦略**
- **9468.T Pullback** は oos の 1.8 倍上振れ = 隠れたエース

### D8b: universe 戦略名整合 (BBShort → BbShort、ORB/Momentum 除外)

`scripts/d8_normalize_universe_strategies.py`:
- `BBShort` (大文字) → `BbShort` に rename (3 件、strategy_factory 整合)
- `ORB` (3 件) と `Momentum5Min` (2 件) を demote
  (strategy_factory に未登録、oos_daily=0 の死んだ entry)
- `active_count: 27 → 22`

### D8c: UNHEALTHY 4 銘柄 demote + lot_multiplier 全面再計算

`scripts/d8_demote_and_recompute_lot_mult.py`:

demote 対象:
- 3103.T Breakout: -244 円/日 (lot_mult=2.72 → demote、最大の改善)
- 6758.T Breakout: -112 円/日
- 1605.T Pullback: -107 円/日
- 8306.T Pullback: -107 円/日

実測値ベース lot_multiplier 再計算後:

| 順位 | 銘柄 | 戦略 | 実測 | mult |
|-----:|------|------|-----:|-----:|
| 1 | 6752.T | MacdRci | +8,348 | 3.00 |
| 2 | 9984.T | EnhancedMacdRci | +3,661 | 2.18 (D7=3.00 から下方修正) |
| 3 | 6501.T | MicroScalp | +2,322 | 1.40 |
| 4 | 1605.T | MicroScalp | +2,289 | 1.38 |
| 5 | 6723.T | MacdRci | +2,154 | 1.30 |

期待 PnL (D8 実測ベース、圧縮 40%、D2 +3,000):
- 機械分散: +14,976 円/日 (50.4%)
- D8 期待値駆動: **+23,388 円/日 (78.7%)**
- D7 楽観値 +41,743 → D8 +23,388 (-18,355 修正、より現実的)

### D8d-e: 6752.T 多戦略追加検証 → EnhancedMacdRci 採用

`scripts/d8_6752_multi_strategy_test.py`:
6752.T に BbShort / Pullback / Breakout / EnhancedMacdRci の 4 戦略を 60d 検証:

| 戦略 | n | wr | pf | pnl/d | verdict |
|------|--:|---:|---:|------:|---------|
| BbShort | 17 | 58.8% | 0.54 | -738 | LOW (大負けあり) |
| Pullback | 48 | 43.8% | 1.02 | +52 | LOW |
| Breakout | 50 | 40.0% | 0.92 | -173 | LOW |
| **EnhancedMacdRci** | **222** | **40.1%** | **1.38** | **+2,034** | **OK** |

→ 6752.T EnhancedMacdRci を universe に追加 (`scripts/d8_add_6752_enhanced.py`)。
6752.T 単独で **+10,382 円/日 = current universe の 32.5%** を占める圧倒的最強銘柄に。

### D8f: 1m データ蓄積パイプライン (PoC)

`scripts/d8_1m_data_pipeline.py`:
- yfinance で過去 7 日の 1m データ取得 → `algo_shared/ohlcv_cache/{sym}_1m.pkl` に追記
- 30 日 retention で自動 trim
- 12 銘柄 × 4 営業日 = ~1,287 行/銘柄を初回蓄積成功
- VPS cron 設定例: `30 16 * * 1-5 python3 scripts/d8_1m_data_pipeline.py`

これにより以下が将来可能に:
- paper SL slippage 仮説の 1m バックテストでの再検証 (D1c)
- MicroScalp の毎日朝 backtest (前日までの 1m 30 日分で per-symbol 最適化)
- Phase 2 動的 lot_multiplier の朝 9:30 動意スコア算出

### D8 サマリ + ROADMAP 連動

- ✅ 全戦略 60d 健全性監査 完了
- ✅ UNHEALTHY 4 銘柄 demote + lot_multiplier 実測値で再校正 → **+23,388 円/日 (78.7%)**
- ✅ 6752.T EnhancedMacdRci 追加 → 6752.T 単独で +10,382 円/日 (universe の 32.5%)
- ✅ 1m データ蓄積パイプライン整備 (PoC)
- 期待 PnL の真値: 機械分散 +14,976 / D8 駆動 +23,388 / 線形効率 80% で +25,000-29,000 円/日
- 5/7 paper で実測 → 実効率 85% 以上なら目標達成、Phase 2 (動的化) 追加で +5,000 円/日 上振れ余地

universe active: 33 (D5) → 27 (D6) → 22 (D8b) → 18 (D8c demote) → **19 (D8e 追加後)**

### Day 8 全変更ファイル

- `data/universe_active.json` (BBShort→BbShort rename, ORB/Momentum demote, UNHEALTHY 4 demote, 6752 EnhancedMacdRci 追加, lot_mult 再計算)
- `scripts/d8_health_check_all_strategies.py` 新規
- `scripts/d8_normalize_universe_strategies.py` 新規
- `scripts/d8_demote_and_recompute_lot_mult.py` 新規
- `scripts/d8_6752_multi_strategy_test.py` 新規
- `scripts/d8_add_6752_enhanced.py` 新規
- `scripts/d8_1m_data_pipeline.py` 新規
- `algo_shared/ohlcv_cache/{sym}_1m.pkl` (12 銘柄、初回蓄積)
